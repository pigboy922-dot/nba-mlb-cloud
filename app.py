...
GOOGLE_SHEET_NAME = os.environ.get("GOOGLE_SHEET_NAME", "results")

# Stable front-end columns. Extra keys are still stored in raw_json.
PREFERRED_COLUMNS = [
...
def canonicalize_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not game_id:
        ...
        game_id = f"{league}_{season}_{game_date}_{matchup or (away + '_vs_' + home)}"
        data["比賽ID"] = game_id
...
def row_to_payload(row: sqlite3.Row) -> Dict[str, Any]:
    try:
        ...
        payload = {}
    if "聯盟" not in payload:
        payload["聯盟"] = row["league"]
    if "賽季" not in payload:
        payload["賽季"] = row["season"]
    if "比賽日期" not in payload:
        payload["比賽日期"] = row["game_date"]
    if "比賽ID" not in payload:
        payload["比賽ID"] = row["game_id"]
    return payload
...
def compute_stats_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # Front-end computes the actual percentages from rows; backend only needs to return rows.
    return rows
...
def get_or_create_worksheet(spreadsheet, title: str):
    try:
        ...
        ws = spreadsheet.add_worksheet(title=title, rows=2000, cols=max(26, len(PREFERRED_COLUMNS) + 10))
...
def maybe_mirror_to_sheet(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if key in row_index:
            ...
            ws.update(f"A{row_num}:{chr(64 + min(len(headers), 26))}{row_num}", [row_values])
            return {"enabled": True, "worksheet": GOOGLE_SHEET_NAME, "action": "update", "row": row_num}
        ws.append_row(row_values, value_input_option="USER_ENTERED")
        return {"enabled": True, "worksheet": GOOGLE_SHEET_NAME, "action": "append"}
...
def proxy_scoreboard(league: str, sport: str, dates: str):
    ...
    resp = requests.get(url, params=params, timeout=ESPN_TIMEOUT, headers={"User-Agent": "Mozilla/5.0"})
...
def root():
    return jsonify({
        ...
        "time": utc_now_iso(),
        "endpoints": ["/health", "/save_result", "/results", "/stats", "/proxy/scoreboard", "/scoreboard"],
    })
...
def results():
    ...
    limit = request.args.get("limit", MAX_RESULTS_DEFAULT)
    try:
        limit_num = max(1, min(int(limit), 20000))
    except Exception:
